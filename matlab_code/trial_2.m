...
while vin < drones+1
    cp= compose("/uav%d/cmd_vel",vin);
    sp=compose("/uav%d/ground_truth_to_tf/pose",vin);
    
    odomSub=[odomSub,rossubscriber(sp)];
    
    chatpub=[chatpub,rospublisher(cp)];
    var_twist =[var_twist,rosmessage(chatpub(vin))];
    ...
    while max(dist)>thresh
        ...
        for i = 1:numel(c) %calculate the centroid of each cell
            ...
            if ~isnan(cx) && inpolygon(cx,cy,crs(:,1),crs(:,2))
                ...
                drone_number_change = compose("/uav%d/action/pose",i);
                
                [client,g]= rosactionclient(drone_number_change,"DataFormat","struct");
                %[client,g]= rosactionclient('/uav1/action/pose','hector_uav_msgs/Pose');
                ...
                gtx(i)=Px(i)*5;
                gty(i)=Py(i)*5;%% postition from voronoi
                
                pose = receive(odomSub(i)).Pose; %%actual position of the drone
                tfx = pose.Position.X;
                tfy = pose.Position.Y;
                tfz = pose.Position.Z;
                    
                velx = -(tfx - gtx(i));  %%% velocitu
                vely = -(tfy - gty(i));
                velz = 3 -tfz;
                var_twist(i).Linear.X = velx;
                var_twist(i).Linear.Y = vely;
                var_twist(i).Linear.Z = velz;
                ...
                %g.target_pose.pose.position.x = gtx(i);
                %g.target_pose.pose.position.y = gty(i);
                %g.target_pose.pose.position.z = 3;

                g.TargetPose.Header.FrameId = 'world';
                g.TargetPose.Pose.Position.X = gtx(i);
                g.TargetPose.Pose.Position.Y = gty(i);
                g.TargetPose.Pose.Position.Z = 5;
                
                %[resultMsg,resultState] = sendGoalAndWait(client,g,1);
                %[resultMsg,resultState] = sendGoal(client,g);
                sendGoal(client,g);
                %pause(0.2);
                %rosShowDetails(resultMsg)
                %send(chatpub(i),var_twist(i));      
                %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
