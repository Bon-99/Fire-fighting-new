...
bridge = CvBridge()            ### We use cv bridge to convert the image from ROS to opencv ###
...
### Function that takes in the goal coordinates and  drone number and moves the drone in gazebo, waiter pauses the code for 0.2 seconds to let the drone move towards the goal ###
def mover(drone_new_x,drone_new_y,drone_num,waiter=True):
    ...
    g.target_pose.pose.position.z = 15
...
### Function that captures an image  and returns bounding box coordinates ###
...
def fire_detection(image,drone_x,drone_y,drone_num):
    ...
    cv2.THRESH_BINARY)                                 # we are using binary thresholding to the image
    ...
    if len(contours) > 0:
        ...
        if (cv2.contourArea(c)) > 5000:
           ...
           #cv2.waitKey(0)
            ...
            img_x = 320                                 # the size of the image captured by the drone is 640, 512
            img_y = 256
            drone_new_x = drone_x
            drone_new_y = drone_y
            if bounding_box_x < img_x:
                while bounding_box_x < img_x :
...
## function that follows the voronoi algorithm, and moves the drone 
...
## moving the drone to a certain height before stating with the voronoi search 
